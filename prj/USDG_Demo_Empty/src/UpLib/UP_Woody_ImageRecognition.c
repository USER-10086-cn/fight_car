 ...
 * @图像捕捉功能使用基本流程：第一步：系统初始化之前调用UP_Woody_Init();初始化Woody模块
 * @图像捕捉能使用基本流程：第二步：调用UP_Woody_StartUp(0x04);启动Woody语音播放功能,(一个字节的第三位为1,0000 0100)
 * @图像捕捉能使用基本流程：第三步：调用UP_Woody_SpeechRecLetIssue();(允许woody下发识别结果)
 * @图像捕捉能使用基本流程：第四步：根据ID或HSI方式设置阈值
 * @图像捕捉能使用基本流程：第五步：自定义（图像捕捉会收到捕捉到的像素点数，X，Y坐标，可根据自己需要编程,例如：像素点数>1000即执行相应操作）
 * @author   nealwang
 * @date     18/12/2015 
 * @version  v1.0 
...
#include "UP_Woody_ImageRecognition.h"
 ...
 //static u8 revoic_flage= 0xac;/********oxac 表示向woody 发送读数据指令*********/
...
//串口使能请放在系统初始化之前，因为使能过程设定了串口的中断入口和波特率
 ...
 /**************************************
	函数名称: UP_Woody_ImageRecLetIssue
  函数功能: 图像识允许下发
...
/**************************************
	函数名称: UP_Woody_ImageRecForbidIssue
  函数功能: 图像识停止下发
  被本函数调用的函数: UP_USR232_Putc或UP_UART5_Putc
  调用本函数的函数: 无    
  输入参数说明: 无
...
/**************************************
	函数名称: UP_Woody_ImageRecHSISet
  函数功能: 图像识别根据hsi设置
  被本函数调用的函数: UP_USR232_Putc或UP_UART5_Putc
  调用本函数的函数: 无    
  输入参数说明: HMax,HMin,SMax,SMin,IMax,IMin
  输出参数说明: 无       
  函数返回值: 无
  其它说明: 无  
**************************************/
