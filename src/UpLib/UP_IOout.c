/******************************************************************/
//	Copyright (C), 2011-2013, 深圳欧比特 
//  Author   	  	: 张浩元  
//  Reviser				: 修改者
//  Update Date   : 2012/08/01
//  Version   	  : 1.3          
//  Description   : Transplant to v3.5 function library
//  Update Date   : 2013/07/09
//  Version   	  : 1.3.1          
//  Description   : modify the UP_Sev_SetALLIO(u8 Value) 
//  Update Date   : 2013/09/24
//  Version   	  : 1.4          
//  Description   : modify to IO out function
//  Update Date   : 2026/08/06
//  Version   	  : 1.5          
//  Description   : 移除 PA13, PA14, PA15 用于 SWD 烧录调试 - IOout0/1 暂不初始化
/******************************************************************/ 

#include "STM32Lib\\stm32f10x.h"
#include "UP_IOout.h"


//IO 端口定义
#define UP_IOout_NUM		6				//IO 输出引脚数目

//IO 口定义 - 已修改：PA13, PA14, PA15 用于烧录调试，改用其他引脚
// IOout0~PA8 (原 PA14 - 已释放用于 SWD/TMS 烧录)
#define UP_IOout0_L		(GPIOA->BSRR = GPIO_Pin_8)
#define UP_IOout0_H		(GPIOA->BRR = GPIO_Pin_8)
// IOout1~PA11 (原 PA15 - 已释放用于 SWD/TCK 烧录；避开 PA9=USART1_TX 蓝牙)
#define UP_IOout1_L		(GPIOA->BSRR = GPIO_Pin_11)
#define UP_IOout1_H		(GPIOA->BRR = GPIO_Pin_11)
// IOout2~PD0
#define UP_IOout2_L		(GPIOD->BSRR = GPIO_Pin_0)
#define UP_IOout2_H		(GPIOD->BRR = GPIO_Pin_0)	
// IOout3~PD1
#define UP_IOout3_L		(GPIOD->BSRR = GPIO_Pin_1)
#define UP_IOout3_H		(GPIOD->BRR = GPIO_Pin_1)	
// IOout4~PD2
#define UP_IOout4_L		(GPIOD->BSRR = GPIO_Pin_3)
#define UP_IOout4_H		(GPIOD->BRR = GPIO_Pin_3)
// IOout5~PD3
#define UP_IOout5_L		(GPIOD->BSRR = GPIO_Pin_4)
#define UP_IOout5_H		(GPIOD->BRR = GPIO_Pin_4)		

// 全局变量

u8  g_UP_IOVal[UP_IOout_NUM];					//IO 端口的电平

/*************************************************
  函数名称：UP_IOout_AllLow    
  函数功能：将所有 IO 口输出低电平
  用到的子函数：无        
  被调用的函数：void UP_IOout_GPIOInit(void);   
  参数说明：无         
  返回值：无       
  备注说明：无
*************************************************/
void UP_IOout_AllLow(void)
{
	UP_IOout0_L; UP_IOout1_L; UP_IOout2_L;
	UP_IOout3_L; UP_IOout4_L; UP_IOout5_L; 
}

/*************************************************
  函数名称：UP_IOout_GPIOInit    
  函数功能：初始化 IO 口
  用到的子函数：void RCC_APB2PeriphClockCmd(uint32_t RCC_APB2Periph, FunctionalState NewState);
                  void GPIO_PinRemapConfig(uint32_t GPIO_Remap, FunctionalState NewState);
                  void UP_IOout_AllLow(void);void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct);
  被调用的函数：无    
  参数说明：无         
  返回值：无       
  备注说明：无
*************************************************/
void UP_IOout_GPIOInit(void)
{
	GPIO_InitTypeDef GPIO_InitStructure;			//定义 GPIO 初始化结构体

	//打开时钟 CLOCK,使用 GPIO 之前必须先打开相应的时钟
	RCC_APB2PeriphClockCmd(
		RCC_APB2Periph_GPIOA|
		RCC_APB2Periph_GPIOD|
		RCC_APB2Periph_GPIOE|
		RCC_APB2Periph_AFIO,
		ENABLE);

	// 修改：注释掉禁用 SWJ 调试端口 - 启用 PA13/PA14/PA15 用于下载调试
	//GPIO_PinRemapConfig(GPIO_Remap_SWJ_Disable,ENABLE);	//禁用调试端口

	// 设置初始状态为低电平
	UP_IOout_AllLow();

	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;	//推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;	//50M 时钟速度

	// PA8, PA11 替代原 PA14, PA15 (PA13/PA14/PA15 留给 SWD 调试；PA9=USART1_TX 蓝牙保留)
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8|GPIO_Pin_11;
	GPIO_Init(GPIOA, &GPIO_InitStructure);

	// PD0~1,PD3~4,PD12,13 等其他 IO
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0|GPIO_Pin_1|GPIO_Pin_3|GPIO_Pin_4;
	GPIO_Init(GPIOD, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_13;
	GPIO_Init(GPIOE, &GPIO_InitStructure);
}

/*************************************************
  函数名称：UP_IOout_Init    
  函数功能：初始化 IOout
  用到的子函数：void UP_IOout_GPIOInit(void);       
  被调用的函数：void UP_System_Init(void);
  参数说明：无         
  返回值：无       
  备注说明：无
*************************************************/
void UP_IOout_Init(void)
{
	UP_IOout_GPIOInit();		//初始化 IO
}

/*************************************************
  函数名称：UP_IOout_SetIO    
  函数功能：单个 IO 口控制
  用到的子函数：无      
  被调用的函数：无
  参数说明：Channel 对应 6 个端口 0~5
			        Value  设置该端口的电平值 0/1       
  返回值：无       
  备注说明：无
*************************************************/
void  UP_IOout_SetIO(u8 Channel, u8 Value)
{
	if(Channel > UP_IOout_NUM || Value > 1)
		return;
	switch(Channel)								//选择对应的端口设置为需要的电平
	{
		case 0: Value?UP_IOout0_H:UP_IOout0_L;break;
		case 1: Value?UP_IOout1_H:UP_IOout1_L;break;
		case 2: Value?UP_IOout2_H:UP_IOout2_L;break;
		case 3: Value?UP_IOout3_H:UP_IOout3_L;break;
		case 4: Value?UP_IOout4_H:UP_IOout4_L;break;
		case 5: Value?UP_IOout5_H:UP_IOout5_L;break;
		default: break;
	}
}

/*************************************************
  函数名称：UP_IOout_SetALLIO   
  函数功能：同时设置 6 路模拟量输入口 IO 输出口电平
  用到的子函数：无      
  被调用的函数：无
  参数说明：Value  设置该端口的电平值 0x00~0x3F       
  返回值：无       
  备注说明：无
*************************************************/
void  UP_IOout_SetALLIO(u8 Value)
{
	u8 i;
	for(i=0; i<8; i++)
	{
		g_UP_IOVal[i] = Value&0x01;
		switch(i)								//选择对应的端口设置为需要的电平
		{
			case 0: Value?UP_IOout0_H:UP_IOout0_L;break;
			case 1: Value?UP_IOout1_H:UP_IOout1_L;break;
			case 2: Value?UP_IOout2_H:UP_IOout2_L;break;
			case 3: Value?UP_IOout3_H:UP_IOout3_L;break;
			case 4: Value?UP_IOout4_H:UP_IOout4_L;break;
			case 5: Value?UP_IOout5_H:UP_IOout5_L;break;	
			default: break;
		}
		Value >>= 1;	
	}
}						
